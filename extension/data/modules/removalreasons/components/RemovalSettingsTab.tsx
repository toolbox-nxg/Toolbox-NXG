...
export function RemovalSettingsTab ({state, saveRef, onSave,}: Props,) {
	...
	function handleSave () {
		...
		state.config.removalReasons = {
			...
			logreason,
			// `nativeSync` and the imported reasons ride through untouched in the `...rr` spread
			// above: the sync toggle lives on the Edit removal reasons tab, and this tab must not
			// quietly rewrite state it no longer owns.
			reasons: rr.reasons || [],
		}
