...
**Auto-expand action table** — automatically expands the action table in queues so it's always visible without a click. A second setting extends that to every page the table appears on (subreddit listings, comments pages, profiles), so you never have to toggle it per item; it's off by default.
...
| Auto-expand action table              | On      | Expand the action table by default in queues                  |
| Auto-expand action table everywhere   | Off     | Expand the action table by default on every page it appears   |
