...
Better Buttons enhances the moderation buttons on old Reddit, adding shortcuts for common combined actions and filling gaps left by Reddit's native UI.

## Overview

The module injects additional buttons and tweaks button behaviour throughout old Reddit — comment threads, post listings, and the mod queue. Each enhancement is independently toggled from the module settings.
...
**Approve on ignore** — automatically approves an item when you click "ignore reports".

**Ignore on approve** — automatically ignores reports when you approve an item.

**Spam button on ham-removed items** — shows a "spam" button on items removed as ham, letting you re-remove them as spam without un-removing first.

**Remove (not spam) button on spam-removed items** — shows a "remove" button on spam-removed items, letting you convert the removal to a regular remove.

**Sticky/unsticky buttons in post listings** — adds sticky and unsticky buttons directly on posts in listing views.

**Comment lock button** — adds a lock/unlock button to individual comments.
